...
- **接口说明：** 交易退款完成时向终端发送通知
- **接口地址：** 通过商户平台配置或发起退款时使用`notice_url`参数配置通知地址
- **请求方式：** POST
- **请求参数：**
    | 参数名称 | 参数类型 | 出现要求 | 描述 |
    | -------- | -------- | -------- | ---- |
    | notice_type | string | C | 通知类型：退款通知固定值为`refund` |
    | trade_no   | string | R | 平台退款流水号 |
    | out_trade_no | string | R | 商家退款流水号 |
    | origin_trade_no | string | R | 退款对应的收款交易流水号 |
